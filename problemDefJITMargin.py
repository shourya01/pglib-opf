...
class opfSocpMargin():
    ...
    def __init__(self, ppc: Dict, casename: str = 'ppc', margin_sense: int = 1):
        
        self.casename = casename
        self.ppc = ppc
        self.margin_sense = margin_sense
    ...
    def generateIndices(self):
        ...
        # input size
        self.in_size = 3*self.n_bus + 6*self.n_branch + 2*self.n_gen
        raw_idx = np.arange(self.in_size)
        raw_vidx = np.split(raw_idx,np.cumsum([self.n_bus,self.n_bus,self.n_bus,self.n_branch,self.n_branch,self.n_branch,self.n_branch,self.n_branch,self.n_branch,self.n_gen,self.n_gen])[:-1])
        self.vidx = {
            'U':raw_vidx[0].astype(int),
            'mPd':raw_vidx[1].astype(int),
            'mQd':raw_vidx[2].astype(int),
            'rW':raw_vidx[3].astype(int),
            'iW':raw_vidx[4].astype(int),
            'rSf':raw_vidx[5].astype(int),
            'iSf':raw_vidx[6].astype(int),
            'rSt':raw_vidx[7].astype(int),
            'iSt':raw_vidx[8].astype(int),
            'rSg':raw_vidx[9].astype(int),
            'iSg':raw_vidx[10].astype(int)
        }
    ...
    def saveJacobianStructure(self):
        ...
        for busid in self.bus_list:
            ...
            jacidx[cons_counter,self.vidx['mPd'][busid]] = 1
        ...
        for busid in self.bus_list:
            ...
            jacidx[cons_counter,self.vidx['mQd'][busid]] = 1
            jacidx[cons_counter,self.vidx['U'][busid]] = 1
            # add contribution from 'from' buses
            for _,ol in self.out_bus[busid]:
                br_a = self.brattr[ol]
    ...
    def jacobianJIT(self):
        ...
        for busid in self.bus_list:
            ...
            idx = self.jacsearch((cons_counter,self.vidx['mPd'][busid]))
            self.jac_coeff[idx]['const'] += -self.margin_sense
        ...
        for busid in self.bus_list:
            ...
            for genid in self.gen_on_bus[busid]:
                ...
                self.jac_coeff[idx]['const'] += 1
            idx = self.jacsearch((cons_counter,self.vidx['mQd'][busid]))
            self.jac_coeff[idx]['const'] += -self.margin_sense
            idx = self.jacsearch((cons_counter,self.vidx['U'][busid]))
    ...
    def hessianJIT(self):
        ...
        # objective's constribution to hessian: None, objective is linear
    ...
    def objective(self,x):
        ...
        mPd, mQd = x[self.vidx['mPd']], x[self.vidx['mQd']]
        
        return - np.sum(mPd) - np.sum(mQd)
    ...
    def gradient(self, x):
        ...
        for busid in self.bus_list:
            grad[self.vidx['mPd'][busid]] = -1
            grad[self.vidx['mQd'][busid]] = -1
    ...
    def constraints(self, x):
        ...
        for busid in self.bus_list:
            ...
            for genid in self.gen_on_bus[busid]:
                cons += rSg[genid]
            cons -= (attr['pd'] + self.margin_sense * mPd[busid])
        ...
        for busid in self.bus_list:
            ...
            for genid in self.gen_on_bus[busid]:
                cons += iSg[genid]
            cons -= (attr['qd'] + self.margin_sense * mQd[busid])
    ...
    def calc_var_bounds(self):
        ...
        # margin
        ub[self.vidx['mPd']] = self.LARGE_NUMBER
        lb[self.vidx['mPd']] = 0
        ub[self.vidx['mQd']] = self.LARGE_NUMBER
        lb[self.vidx['mQd']] = 0
    ...
    def vars_calculator(self,x):
        ...
        mPd, mQd = x[self.vidx['mPd']], x[self.vidx['mQd']] # not used
