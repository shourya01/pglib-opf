...
class opfSocpR():
    ...
    def generateIndices(self):
        ...
        # constraints
        self.cons_size = 2*self.n_bus + 5*self.n_branch + self.idx_ffr.sum() + self.idx_ftr.sum() + self.idx_aminr.sum() + self.idx_amaxr.sum()
        raw_coidx = np.arange(self.cons_size)
        raw_cidx = np.split(raw_coidx,np.cumsum([self.n_bus,self.n_bus,self.n_branch,self.n_branch,self.n_branch,self.n_branch,
                self.idx_ffr.sum(),
                self.idx_ftr.sum(),
                self.idx_aminr.sum(),
                self.idx_amaxr.sum(),
    ...
    def saveJacobianStructure(self):
        ...
        # from flow limits
        for bidx,(_,brid) in enumerate(self.branch_list):
            if self.idx_ffr[bidx] == 0:
                continue
            attr = self.brattr[brid]
            jacidx[cons_counter,self.vidx['rSf'][attr['idx']]] = 1
            jacidx[cons_counter,self.vidx['iSf'][attr['idx']]] = 1
    ...
    def constraintsJIT(self):
        
        self.const_not_loads = []
        self.linear_fac = lil_matrix((self.cons_size,self.in_size))
        self.sq_fac = lil_matrix((self.cons_size,self.in_size))
        self.bi1_fac = lil_matrix((self.cons_size,self.in_size))
        self.bi2_fac = lil_matrix((self.cons_size,self.in_size))
        cons_counter = 0
        
        # real balance
        for busid in self.bus_list:
            attr = self.busattr[busid]
            # populate constant factor ----
            self.const_not_loads.append(0)
            # populate linear factor ----
            for genid in self.gen_on_bus[busid]:
                self.linear_fac[cons_counter,self.vidx['rSg'][genid]] = 1
            self.linear_fac[cons_counter,self.vidx['U'][busid]] = -attr['gs']
            # add contribution 'from' buses
            for _,ol in self.out_bus[busid]:
                br_a = self.brattr[ol]
                self.linear_fac[cons_counter,self.vidx['rSf'][br_a['idx']]] = -1
            # add contribution 'to' buses
            for _,il in self.in_bus[busid]:
                br_a = self.brattr[il]
                self.linear_fac[cons_counter,self.vidx['rSt'][br_a['idx']]] = -1
            # populate square factor ---- NONE
            # populate bilinear 1 factor ---- NONE
            # populate bilinear 2 factor ---- NONE
            # update
            cons_counter += 1
        ...
        for busid in self.bus_list:
            ...
            attr = self.busattr[busid]
            for genid in self.gen_on_bus[busid]:
                self.linear_fac[cons_counter,self.vidx['iSg'][genid]] = 1
            self.linear_fac[cons_counter,self.vidx['U'][busid]] = attr['bs']
            # add contribution 'from' buses
            qfbus, qtbus = 0, 0
            for _,ol in self.out_bus[busid]:
                br_a = self.brattr[ol]
                self.linear_fac[cons_counter,self.vidx['iSf'][br_a['idx']]] = -1
            # add contribution 'to' buses
            for _,il in self.in_bus[busid]:
                br_a = self.brattr[il]
                self.linear_fac[cons_counter,self.vidx['iSt'][br_a['idx']]] = -1
            # populate square factor ---- NONE
            # populate bilinear 1 factor ---- NONE
            # populate bilinear 2 factor ---- NONE
            # update
        ...
        for _,brid in self.branch_list:
            ...
            # populate linear factor ----
            attr = self.brattr[brid]
            self.linear_fac[cons_counter,self.vidx['rSf'][brid]] = 1
            self.linear_fac[cons_counter,self.vidx['U'][attr['f']]] = -attr['yff'].real
            self.linear_fac[cons_counter,self.vidx['rW'][attr['idx']]] = -attr['yft'].real
            self.linear_fac[cons_counter,self.vidx['iW'][attr['idx']]] = -attr['yft'].imag
            # populate square factor ---- NONE
            # update
            cons_counter += 1
        ...
        for _,brid in self.branch_list:
            ...
            # populate linear factor ----
            attr = self.brattr[brid]
            self.linear_fac[cons_counter,self.vidx['iSf'][brid]] = 1
            self.linear_fac[cons_counter,self.vidx['U'][attr['f']]] = attr['yff'].imag
            self.linear_fac[cons_counter,self.vidx['iW'][attr['idx']]] = -attr['yft'].real
            self.linear_fac[cons_counter,self.vidx['rW'][attr['idx']]] = attr['yft'].imag
            # populate square factor ---- NONE
            # populate bilinear 1 factor ---- NONE
            # populate bilinear 2 factor ---- NONE
            # update
        ...
        for _,brid in self.branch_list:
            ...
            # populate linear factor ----
            attr = self.brattr[brid]
            self.linear_fac[cons_counter,self.vidx['rSt'][brid]] = 1
            self.linear_fac[cons_counter,self.vidx['U'][attr['t']]] = -attr['ytt'].real
            self.linear_fac[cons_counter,self.vidx['rW'][attr['idx']]] = -attr['ytf'].real
            self.linear_fac[cons_counter,self.vidx['iW'][attr['idx']]] = attr['ytf'].imag
            # populate square factor ---- NONE
            # populate bilinear 1 factor ---- NONE
            # populate bilinear 2 factor ---- NONE
            # update
        ...
        for _,brid in self.branch_list:
            ...
            # populate linear factor ----
            attr = self.brattr[brid]
            self.linear_fac[cons_counter,self.vidx['iSt'][brid]] = 1
            self.linear_fac[cons_counter,self.vidx['U'][attr['t']]] = attr['ytt'].imag
            self.linear_fac[cons_counter,self.vidx['iW'][attr['idx']]] = attr['ytf'].real
            self.linear_fac[cons_counter,self.vidx['rW'][attr['idx']]] = attr['ytf'].imag
            # populate square factor ---- NONE
            # populate bilinear 1 factor ---- NONE
            # populate bilinear 2 factor ---- NONE
            # update
        ...
        for bidx,(_,brid) in enumerate(self.branch_list):
            ...
            attr = self.brattr[brid]
            # populate constant factor ----
            self.const_not_loads.append(-math.pow(attr['fmax'],2))
            # populate linear factor ---- NONE
            # populate square factor ---- 
            self.sq_fac[cons_counter,self.vidx['rSf'][brid]] = 1
            self.sq_fac[cons_counter,self.vidx['iSf'][brid]] = 1
            # populate bilinear 1 factor ---- NONE
            # populate bilinear 2 factor ---- NONE
            # update
            cons_counter += 1
        ...
        for bidx,(_,brid) in enumerate(self.branch_list):
            ...
            # populate linear factor ---- NONE
            # populate square factor ---- 
            self.sq_fac[cons_counter,self.vidx['rSt'][brid]] = 1
            self.sq_fac[cons_counter,self.vidx['iSt'][brid]] = 1
            # populate bilinear 1 factor ---- NONE
            # populate bilinear 2 factor ---- NONE
            # update
            cons_counter += 1
        ...
        for bidx,(_,brid) in enumerate(self.branch_list):
            ...
            self.const_not_loads.append(0)
            # populate linear factor ---- 
            self.linear_fac[cons_counter,self.vidx['iW'][brid]] = 1
            self.linear_fac[cons_counter,self.vidx['rW'][brid]] = -np.tan(attr['angmax'])
            # populate square factor ---- NONE
            # populate bilinear 1 factor ---- NONE
            # populate bilinear 2 factor ---- NONE
            # update
        ...
        for bidx,(_,brid) in enumerate(self.branch_list):
            ...
            # populate linear factor ---- 
            self.linear_fac[cons_counter,self.vidx['iW'][brid]] = -1
            self.linear_fac[cons_counter,self.vidx['rW'][brid]] = np.tan(attr['angmin'])
            # populate square factor ---- NONE
            # populate bilinear 1 factor ---- NONE
            # populate bilinear 2 factor ---- NONE
            # update
        ...
        for _,brid in self.branch_list:
            ...
            # populate linear factor ---- NONE
            # populate square factor ----
            self.sq_fac[cons_counter,self.vidx['rW'][brid]] = 1
            self.sq_fac[cons_counter,self.vidx['iW'][brid]] = 1
            # populate bilinear 1 factor ----
            self.bi1_fac[cons_counter,self.vidx['U'][attr['f']]] = -1
            # populate bilinear 2 factor ----
            self.bi2_fac[cons_counter,self.vidx['U'][attr['t']]] = 1
            # update
            cons_counter += 1
            
        self.const_not_loads = np.array(self.const_not_loads)
        self.linear_fac = self.linear_fac.tocsr()
        self.sq_fac = self.sq_fac.tocsr()
        self.bi1_fac = self.bi1_fac.tocsr()
        self.bi2_fac = self.bi2_fac.tocsr()
    ...
    def constraints(self, x):
        ...
        return self.const_not_loads + self.linear_fac.dot(x).ravel() + self.sq_fac.dot(np.square(x)).ravel() \
    ...
    def refresh_loads_for_constr(self):
        ...
        for busid in self.bus_list:
            ...
            pd.append(attr['pd'])
            qd.append(attr['qd'])
        self.loads_for_cons = np.concatenate([np.array(pd),np.array(qd),np.zeros(self.cons_size-2*self.n_bus)],axis=0)
